...
$Accounting = New-Object System.Management.Automation.Host.ChoiceDescription '&Accounting', 'Get running AccountingDL'
$Quit = New-Object System.Management.Automation.Host.ChoiceDescription '&Quit', 'Quit menu'
$options = [System.Management.Automation.Host.ChoiceDescription[]]($CSM, $HR, $Legal, $Marketing, $Partnership, $Product, $Sales, $SolutionsConsulting, $Tech, $Corp, $Support, $Crea, $Accounting, $Quit)
...
Add-TeamUser -GroupId $GroupId -User "$User" -Role "$Role"
       If($?)
{
Write-Host $User Succesfully added to Teams Group -ForegroundColor Green
}
else
{
Write-Host -ErrorAction Stop -ForegroundColor Red
...
Add-MailboxPermission -Identity "shared.accounting@sociabble.com" -User $User -AccessRights FullAccess
if ($?) {
Write-Host "$User has been added to shared accounting mailbox" -ForegroundColor Green
}
else {
Write-Host "An error occured while adding $User to shared accounting mailbox" -ForegroundColor Red
}

Add-RecipientPermission -Identity "shared.accounting@sociabble.com" -Trustee $User -AccessRights SendAs
if ($?) {
Write-Host "$User has been granted SendAs permission on shared accounting mailbox" -ForegroundColor Green
}
else {
Write-Host "An error occured while granting SendAs permission to $User on shared accounting mailbox" -ForegroundColor Red
}


Add-MailboxPermission -Identity "shared.billing@sociabble.com" -User $User -AccessRights FullAccess
if ($?) {
Write-Host "$User has been added to shared billing mailbox" -ForegroundColor Green
}
else {
Write-Host "An error occured while adding $User to shared billing mailbox" -ForegroundColor Red
}

Add-RecipientPermission -Identity "shared.billing@sociabble.com" -Trustee $User -AccessRights SendAs
if ($?) {
Write-Host "$User has been granted SendAs permission on shared billing mailbox" -ForegroundColor Green
}
else {
Write-Host "An error occured while granting SendAs permission to $User on shared billing mailbox" -ForegroundColor Red
}

Add-MailboxPermission -Identity "shared.purchase@sociabble.com" -User $User -AccessRights FullAccess
if ($?) {
Write-Host "$User has been added to shared purchase mailbox" -ForegroundColor Green
}
else {
Write-Host "An error occured while adding $User to shared purchase mailbox" -ForegroundColor Red
}

Add-RecipientPermission -Identity "shared.purchase@sociabble.com" -Trustee $User -AccessRights SendAs
if ($?) {
Write-Host "$User has been granted SendAs permission on shared purchase mailbox" -ForegroundColor Green
}
else {
Write-Host "An error occured while granting SendAs permission to $User on shared purchase mailbox" -ForegroundColor Red
}

Add-MailboxPermission -Identity "shared.rocinante@sociabble.com" -User $User -AccessRights FullAccess
if ($?) {
Write-Host "$User has been added to shared rocinante mailbox" -ForegroundColor Green
}
else {
Write-Host "An error occured while adding $User to shared rocinante mailbox" -ForegroundColor Red
}

Add-RecipientPermission -Identity "shared.rocinante@sociabble.com" -Trustee $User -AccessRights SendAs
if ($?) {
Write-Host "$User has been granted SendAs permission on shared rocinante mailbox" -ForegroundColor Green
}
else {
Write-Host "An error occured while granting SendAs permission to $User on shared rocinante mailbox" -ForegroundColor Red
}
}

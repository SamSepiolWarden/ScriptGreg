...
# Function to add user to distribution lists
function Add-UserToDLs {
    param (
        [string]$FilePath,
        [string]$User
    )
...
# Function to add user to Teams groups
function Add-UserToTeams {
    param (
        [string]$FilePath,
        [string]$User,
        [string]$UserID
    ...
    $TeamsAdd = Import-Csv -Path $FilePath
    $i = 0
...
Try {
    $UserEmail = Get-Mailbox -Identity $User -ErrorAction Stop | Select-Object -ExpandProperty PrimarySmtpAddress
} Catch {
...
$UserID = Get-MgUser -Filter "mail eq '$UserEmail'"
...
Add-UserToDLs -FilePath "C:\Users\GregorySemedo\Desktop\Script\DL\DL-$Location.csv" -User $User
...
Add-UserToDLs -FilePath "C:\Users\GregorySemedo\Desktop\Script\DL\DL-$Department.csv" -User $User

# Connect to Teams and add the user to Teams groups
Connect-MicrosoftTeams
Add-UserToTeams -FilePath "C:\Users\GregorySemedo\Desktop\Script\Ms-Teams-Script\Teams\Teams-$Department.csv" -User $User -UserID $UserID.Id
...
if ($Location -eq 'Paris' -or $Location -eq 'Lyon' -or $Location -eq 'Mumbai') {

...
$User = Read-Host -Prompt "Enter the mail of the user"
...
foreach ($Id in $SearchTeam) {
    ...
    foreach ($tag in $teamTags) {
        if ($tag.DisplayName -eq "OnlySDRs"){
            $tagFound = $true
            try {
                New-MgTeamTagMember -TeamId $Id.Id -TeamworkTagId $tag.Id -UserId $UserID
    ...
    if (-not $tagFound) {
        Write-Host "No tag OnlySales found" -ForegroundColor Gray
    }
}

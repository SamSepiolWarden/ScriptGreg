...
foreach ($Id in $SearchTeam) {
    ...
    foreach ($tag in $teamTags) {
        if ($tag.DisplayName -eq "OnlyCS-Business"){
            $tagFound = $true
            try {
                New-MgTeamTagMember -TeamId $Id.Id -TeamworkTagId $tag.Id -UserId $User
                Write-Host "User successfully added to $tag.DisplayName" -ForegroundColor Green
            }
            catch {
    ...
    if (-not $tagFound) {
        Write-Host "No tag OnlyCS-Business found" -ForegroundColor Gray
    }
}

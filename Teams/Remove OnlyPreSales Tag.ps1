...
foreach ($Id in $SearchTeam) {
    ...
    foreach ($tag in $teamTags) {
        if ($tag.DisplayName -eq "OnlyPresales"){
            $tagFound = $true
            try {
                Remove-MgTeamTagMember -TeamId $Id.Id -TeamworkTagId $tag.Id -UserId $User
                Write-Host "User successfully remove to tag" -ForegroundColor Green
            }
            catch {
                Write-Host "Failed to remove user to tag: $_" -ForegroundColor Red
            }
        }
    }
